...
function Get-SPOTrimEngine {
    if ($null -eq $script:Engine) {
        $dbFolder = Join-Path ([Environment]::GetFolderPath('ApplicationData')) 'LiebenConsultancy' 'SPOTrim'
        if (-not (Test-Path $dbFolder)) { New-Item -ItemType Directory -Path $dbFolder -Force | Out-Null }
        $dbPath = Join-Path $dbFolder 'data.db'
        $script:Engine = [SPOTrim.Engine.Engine]::new($dbPath)
    }
...
if ([Environment]::UserInteractive -and -not [Environment]::GetCommandLineArgs().Contains('-NonInteractive') -and -not $env:SPOTRIM_NO_AUTOSTART) {
    try {
